...
void Shader::initGeo(const std::string& vertFile, const std::string& geoFile, const std::string& fragFile)
{
	shaderID = glCreateProgram(); // create shader program (openGL saves as ref number)
	shaders[0] = CreateShader(LoadShader(vertFile), GL_VERTEX_SHADER); // create vertex shader
	shaders[1] = CreateShader(LoadShader(geoFile), GL_GEOMETRY_SHADER); // create fragment shader
